"""Keyword-classifier data tables.

Split out from `modules/keyword_classifier.py` because the data tables grew
past the 800-line file cap while the classification logic itself is under 200
lines. Splitting keeps both halves reviewable: data changes are purely regex
edits, logic changes never accidentally touch a pattern.

All names remain underscore-prefixed because they are private to the
classifier module; external consumers import from keyword_classifier, not
here.
"""
